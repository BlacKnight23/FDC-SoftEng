...
namespace Accounting
{
    public partial class USC_user : UserControl
    {
        ...
        {
            conn.Open();
            adp = new MySqlDataAdapter("SELECT * FROM user WHERE user_firstname like '" + textBox_search.Text + "%' OR user_lastname like '" + textBox_search.Text + "%' OR user_username like '" + textBox_search.Text + "%' OR user_password like '" + textBox_search.Text + "%' OR user_type like '" + textBox_search + " %'", conn);
            dt = new DataTable();
            adp.Fill(dt);
            dataGridView1.DataSource = dt;
            conn.Close();
